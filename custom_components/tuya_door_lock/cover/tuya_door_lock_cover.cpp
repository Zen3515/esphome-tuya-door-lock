...
static const char *const TAG = "tuya_door_lock.cover";

void TuyaDoorLockCover::setup() {
  ...
  this->parent_->register_listener(report_id, [this](const TuyaDoorLockDatapoint &datapoint) {
...
void TuyaDoorLockCover::control(const cover::CoverCall &call) {
  if (call.get_stop()) {
...
void TuyaDoorLockCover::set_direction_(bool inverted) {
...
void TuyaDoorLockCover::dump_config() {
  ESP_LOGCONFIG(TAG, "TuyaDoorLock Cover:");
...
cover::CoverTraits TuyaDoorLockCover::get_traits() {
